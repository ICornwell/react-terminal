...
export default async function exec(commands: [string], text: string,
     errorMessage: any, question: any) {
    
    const finalText: string = question && !question.isAnswered? resolveText(question, text) : text    
...
async function execNamedCommand(commands: [string], command: string, commandArguments: string) {
    const executor = commands[command];
    if (typeof executor === "function") 
        return await executor(commandArguments);
    else 
        return executor;
}
...
function resolveText(question: any, text: string): string {
    const answer = question.answers.find(a=>a.text.toLowerCase() === text.toLowerCase())
